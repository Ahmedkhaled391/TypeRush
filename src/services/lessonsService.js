...
export function getPassedCount() {
  if (progressCache) {
    return Object.values(progressCache.lessonMeta).filter(
      (entry) => entry.passed,
    ).length;
  }
...
export async function submitLessonAttempt({
  ...
  const stars =
    result.stars ??
    calculateStars(wpm, accuracy, wpmRequirement, accuracyRequirement);
  ...
  if (progressCache) {
    const current = progressCache.lessonMeta[lessonId] || {
      passed: false,
      bestStars: 0,
    };
    ...
    progressCache.unlockedUpTo = recalculateUnlockedUpTo(
      progressCache.lessonMeta,
    );
  }
